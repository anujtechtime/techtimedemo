...
class GeneralView(models.TransientModel):
    ...
    def _get_accounts(self, accounts, init_balance, display_account, data):
        ...
        if init_balance and data.get('date_from'):
            ...
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, COALESCE(SUM(l.debit / 1310),0.0) AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
        ...
        sql = ('''SELECT  l.account_id AS account_id, a.code AS code,a.id AS id, a.name AS name, ROUND(COALESCE(SUM(l.debit),0),2) AS debit, ROUND(COALESCE(SUM(l.credit),0),2) AS credit, ROUND(COALESCE(SUM(l.balance),0),2) AS balance
               ...
               + WHERE + new_final_filter + ''' GROUP BY l.account_id, a.code, a.name, a.id ORDER BY a.code''')
        if data.get('accounts'):
    ...
    def get_accounts_line(self, account_id, title):
        ...
        # Get move lines base on sql query and Calculate the total balance of move lines
        sql = ('''SELECT l.id AS lid,m.id AS move_id, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, l.currency_id,  credit,COALESCE(SUM(l.balance / 1310),0)   AS lamountcurrency, l.ref AS lref, l.name AS lname, COALESCE(SUM(l.debit),0) AS debit,  credit, COALESCE(SUM(l.balance / 1310),0) AS amount_currency, COALESCE(SUM(l.credit),0) AS credit, COALESCE(SUM(l.balance),0) AS balance,\
