...
class TBXLSXReportController(http.Controller):
    ...
    @http.route('/new_dynamic_xlsx_reports', type='http', auth='user', methods=['POST'], csrf=False)
    def new_get_report_xlsx(self, model, options, output_format, report_data, report_name, dfr_data, **kw):
        uid = request.session.uid
        report_obj = request.env[model].with_user(uid)
        dfr_data = dfr_data
        options = options
        ...
        try:
            if output_format == 'xlsx':
                ...
                report_obj.new_get_dynamic_xlsx_report(options, response, report_data, dfr_data)
